 ...
 *   radius            CSS length for --yantra-r. Default clamps between 150px and 270px.
 *   showLines         Render the one-line teachings under core labels (hide on very small wheels).
 */
export default function SriYantra({ onNavigate, radius, showLines = true }) {
  const R = radius ?? "clamp(150px, min(19vw, 30vh), 270px)";
      ...
      <div className="animate-breathe absolute inset-0">
        ...
        {chakras.map((c, i) => {
          ...
          const labelStyle = {
            ...
            gap: "0.3em",
            width: side === "center" ? "calc(var(--yantra-r) * 0.8)" : "calc(var(--yantra-r) * 0.58)",
