...
/** Modal grid picker over the Media Library — click an image to select its URL. Opens pre-filtered to the page's own folder when one matches. */
export default function MediaPickerModal({ open, onClose, onSelect, initialFolder = "All" }) {
  ...
  const items = (folder === "All" ? media : media.filter((m) => m.folder === folder)).filter(
    (m) => !m.mime_type?.startsWith("video")
  );

  return (
    <Modal open={open} onClose={onClose} title="Choose an image" description="Pulled from the Media Library — upload new files there first." size="lg">
      <div className="flex flex-col gap-4">
        ...
        {loading ? (
          ...
          <p className="py-10 text-center text-[13px] text-[var(--a-text-muted)]">
            No images in this folder yet — upload one from the Media Library first.
          </p>
        ) : (
